...
static int user_set_duty(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **) &set_args);
    ...
    const char *type = set_args.type->sval[0];
    const char *values = set_args.value->sval[0];
    _pwm_duty  = strtoul(values, NULL, 0);
    duty_pwm = _pwm_duty;
    ESP_LOGI(TAG, "pwm_duty '%d'", duty_pwm);
    return 0;
}
...
void register_nvs(void)
{
    ...
    const esp_console_cmd_t set_duty = {
        .command = "user_set_duty",
        .help = "set duty motor",
        .hint = NULL,
        .func = &user_set_duty,
        .argtable = &get_args
    ...
    ESP_ERROR_CHECK(esp_console_cmd_register(&set_duty));
}
